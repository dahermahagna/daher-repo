...
static int InitSavedAddressesImp(Dhcp_ty *dhcp)
{
    ...
    while (0 < heigt--)
    {
        ...
        if (NULL == left_runner->relatives[ZERO] || NULL == right_runner->relatives[ONE])
        {
            return -1;
        }
...
static size_t CountTakenHostsImp(Dhcp_node_ty *node, size_t height)
{
    ...
    return CountTakenHostsImp(node->relatives[ZERO], height - 1)
        + CountTakenHostsImp(node->relatives[ONE], height - 1);
}
...
static void UpdateFullnessImp(Dhcp_node_ty *node)
{
    ...
    if (0 == node->relatives[ZERO]->is_full || 0 == node->relatives[ONE]->is_full)
    {
        node->is_full = 0;
        return;
    }
